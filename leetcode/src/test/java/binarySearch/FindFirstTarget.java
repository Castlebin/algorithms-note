...
/**
 * 在正排序数组中，寻找第一个等于 target 的元素的位置，不存在的话返回 -1 (找左边界)
 *
 * 验证方法：LeetCode 34. 在排序数组中查找元素的第一个和最后一个位置
 */
public class FindFirstTarget {

    public int findFirstTargetPos(int[] nums, int target) {
        int left = 0;
        ...
        while (left <= right) {
            ...
            if (nums[mid] == target) {
                // 找到了，如果是第一个，或者前一个不是 target，那么就是第一个
    ...
    public void test() {
        for (int testCount = 0; testCount < 10; testCount++) {
            ...
            int binarySearchResult = findFirstTargetPos(nums, target);
